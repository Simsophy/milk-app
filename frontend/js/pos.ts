    ...
    private async processPayment(order: any): Promise<void> {
        try {
            ...
            if (qrResponse.success) {
                const qrToken = qrResponse.data.qr_token;
                
                // Show success and redirect
                this.showAlert('Order created successfully! Redirecting to payment...', 'success');
                
                setTimeout(() => {
                    // For now, show payment confirmation
                    alert(`Order #${order.id} created with total: $${order.total_amount.toFixed(2)}\n\nQR Token: ${qrToken || 'pending'}`);
                    
                    // Clear cart and reset
                    this.cart = { items: [], total_amount: 0 };
                    this.renderCart();
                    document.getElementById('orderNotes')?.clearTimeout;
                }, 1500);
            } else {
                this.showAlert('Payment QR generation failed', 'error');
            }
        } catch (error) {
